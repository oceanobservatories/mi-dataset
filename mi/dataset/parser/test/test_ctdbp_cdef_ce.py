...
simple_test.log
  Contains a header line + 5 Sensor records

simple_test_meta.log
  Contains complete Header + 5 Sensor records

data1.log
  Contains complete Header + 143 Sensor records
...
RESOURCE_PATH = os.path.join(BASE_RESOURCE_PATH, 'ctdbp_cdef', 'ce', 'resource')

MODULE_NAME = 'mi.dataset.parser.ctdbp_cdef_ce'

SIMPLE_LOG_FILE = "simple_test.log"
SIMPLE_LOG_FILE_HEADER = "simple_test_meta.log"
...
class CtdbpCdefCeParserUnitTestCase(ParserUnitTestCase):
    ...
    def test_verify_record_with_header(self):
        """
        Test that header lines are ignored and data particles are generated for all records
        """
        log.debug('===== START HEADER TEST =====')
        in_file = self.open_file(SIMPLE_LOG_FILE_HEADER)
        parser = self.create_rec_parser(in_file)
    ...
    def extract_data_particle_from_log_file(self):
        ...
        for line in in_file:
            ...
            try:
                ...
                oxygen = str(int(match.group('oxygen'), 16))
