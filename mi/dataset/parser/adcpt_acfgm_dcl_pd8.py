    ...
    def parse_file(self):
        ...
        while True:
            ...
            if dcl_log_match is None:
                ...
                if dcl_log_match is None:
                    self.recov_exception_callback('Unknown data found in line %s' % line)
                # Else verified to be a regular DCL Log. Discard & move to next line.
                continue

            # First timestamp from the DCL controller log
            parsed_data = [dcl_log_match.groups()[SENSOR_GROUP_TIMESTAMP], ]
            line = self._stream_handle.readline()
            ...
            while True:
                ...
                if line_match is not None:
                    ...
                    if line_match is None:
                        self.recov_exception_callback("Expecting empty DCL Timestamp, received: %r" % line)
                        break
